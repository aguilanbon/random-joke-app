...
function ThumbButton() {
    ...
    const { setThumbsDown, setThumbsUp, setLiked, currentJoke, liked, disliked, setDisliked, setIsLiked } = useContext(GlobalContext)
    ...
    const handleThumbAction = (type) => {
        ...
        if(type === 'up') {
            ...
            setDisliked([...disliked, currentJoke])
            setIsLiked(false)
        }
    }
    ...
    return (
        <div className='w-full flex mt-8 justify-center items-center'>
            {isClicked === 'none' ? <motion.h1 animate={{ opacity: [0,1], transition: { duration: 2 } }}>Move along.</motion.h1> : 
                <>
